  ...
  const exerciseResults = useMemo(() => {
    ...
    return rows
      .filter(({ node, exercise }) => {
        if (!exerciseResetState(logsByNodeId[node.id] ?? [], exercise.id, exercise.cadence).available) {
          return false;
        }
    ...
    >
      ...
      >
        <div className="flex items-center gap-3 border-b border-white/10 px-4 py-3">
          ...
          />
          <kbd className="rounded border border-white/10 bg-white/[0.03] px-1.5 py-0.5 font-mono text-[9px] text-neutral-600">Esc</kbd>
        </div>
        ...
        <div className="max-h-[62vh] overflow-y-auto p-2">
          {(commandMatches("analytics dashboard") ||
            ...
            <div className="mb-2">
              <div className="px-2 pb-1 pt-1 text-[9px] font-semibold uppercase tracking-[0.18em] text-neutral-600">Commands</div>
              {commandMatches("analytics dashboard") && (
                <button
                  type="button"
                ...
                >
                  <span className="grid h-7 w-7 place-items-center rounded-lg border border-white/10 bg-white/[0.03]">⌁</span>
              ...
              {selectedId && commandMatches("focus mode") && (
                ...
                >
                  <span className="grid h-7 w-7 place-items-center rounded-lg border border-white/10 bg-white/[0.03]">◎</span>
                  <span className="flex-1">Turn Focus Mode {focusMode ? "off" : "on"}</span>
                  <span className="text-[9px] text-neutral-600">view</span>
                </button>
              )}
            </div>
          ...
          {nodeResults.length > 0 && (
            <div className="mb-2 border-t border-white/6 pt-2">
              <div className="px-2 pb-1 pt-1 text-[9px] font-semibold uppercase tracking-[0.18em] text-neutral-600">Faculties</div>
                ...
                return (
                  ...
                  >
                    ...
                    <span className="min-w-0 flex-1">
                      <span className="block truncate text-xs font-medium text-neutral-200">{node.label}</span>
                      <span className="block truncate text-[10px] text-neutral-600">{category?.label}</span>
                    </span>
          ...
          {exerciseResults.length > 0 && (
            <div className="border-t border-white/6 pt-2">
              <div className="px-2 pb-1 pt-1 text-[9px] font-semibold uppercase tracking-[0.18em] text-neutral-600">Ready XP tasks</div>
              {exerciseResults.map(({ node, exercise }) => {
                ...
                return (
                  <button
                    ...
                    className="flex w-full items-center gap-3 rounded-xl px-3 py-2.5 text-left transition-colors hover:bg-white/[0.06]"
                  >
                    <span className="grid h-7 w-7 shrink-0 place-items-center rounded-lg border border-emerald-300/15 bg-emerald-300/[0.04] text-[10px] text-emerald-200/80">+{award}</span>
                    <span className="min-w-0 flex-1">
                      <span className="block truncate text-xs text-neutral-300">{exercise.label}</span>
                      <span className="block truncate text-[10px] text-neutral-600">{node.label} · {exercise.cadence}</span>
                    </span>
          ...
          {nodeResults.length === 0 && exerciseResults.length === 0 && normalized && (
            <div className="px-4 py-10 text-center text-xs text-neutral-600">No matching faculties, ready tasks, or commands.</div>
          )}
        </div>
